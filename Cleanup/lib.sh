...
CleanupDo() {
  ...
  for t in '/tmp' "$BEAKERLIB_DIR" "~"; do
    ...
    rlLogError "could not create temp file in $t, proceeding with next possible location"
  done
  [[ -n "$t" ]] && {
    rlLogFatal "could not write to temp, thus cleanup was not executed"
    return 1
  }
...
CleanupSetStack() {
  ...
  rlLogFatal "could not set cleanup stack"
  return 1
